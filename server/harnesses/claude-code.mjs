...
/**
 * Resume a thread headlessly, streaming NDJSON over stdio instead of opening the desktop app.
 *
 * Only a CLI-originated session id can be resumed this way — the desktop app's own ids have
 * no CLI-side equivalent, and there is no deep-link fallback the way `openThread` has one, so
 * a desktop-only thread is refused outright rather than silently degrading. `--permission-mode
 * acceptEdits` auto-approves file edits but leaves anything riskier (shell commands, etc.)
 * needing interactive approval it cannot get here — it will stop and report rather than act.
 */
async function driveThread(ref) {
  const { cliSessionId, cwd } = ref || {}
  if (!isCliId(cliSessionId)) return { ok: false, error: 'That thread has no CLI session to resume' }
  if (typeof cwd !== 'string' || !cwd) return { ok: false, error: 'That thread has no folder on record to resume in' }

  const bin = await cliBinary()
  if (!bin) return { ok: false, error: 'The claude CLI is not on this machine' }
  ...
  return {
    ...
    command: {
      argv: [
        ...
        '--verbose', // required by the CLI whenever --print pairs with --output-format=stream-json
      ],
      cwd,
    },
  }
