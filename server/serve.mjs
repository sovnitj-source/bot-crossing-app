...
import { handleDriveUpgrade } from './lib/api-drive.mjs'
import { announceDriveToken } from './lib/security.mjs'
import { shutdownAll } from './lib/drive.mjs'
...
server.on('upgrade', handleDriveUpgrade)

const DATA_DIR = process.env.BOT_CROSSING_DATA || path.join(here, '..', 'data')

server.listen(PORT, HOST, async () => {
  console.log(`Bot Crossing → http://${HOST}:${PORT}`)
  await announceDriveToken(DATA_DIR)
})
